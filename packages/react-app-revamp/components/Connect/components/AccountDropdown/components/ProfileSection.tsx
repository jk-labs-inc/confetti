        ...
        <div className="flex flex-col gap-2 flex-1 min-w-0">
          <div className="flex flex-col gap-1">
            <div className="flex items-center justify-between gap-2">
              <p className="text-[16px] font-bold text-neutral-11 truncate">{ensName || displayName}</p>
              {onAdvancedClick && (
                <button
                  onClick={onAdvancedClick}
                  aria-label="Advanced options"
                  aria-expanded={isAdvancedOpen}
                  className={`shrink-0 transition-transform duration-500 ease-out hover:rotate-[360deg] ${
                    isAdvancedOpen ? "text-neutral-11" : "text-neutral-9 hover:text-neutral-11"
                  }`}
                >
                  <Cog8ToothIcon className="w-[18px] h-[18px]" />
                </button>
              )}
            </div>
            <button
