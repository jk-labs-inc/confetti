export const MOBILE_BREAKPOINT = 768;

export const CHIP_GAP = 10;
export const CHIP_PEEK = 34;
export const CHIP_W_CSS_MOBILE = `calc((100% - ${CHIP_GAP}px - ${CHIP_PEEK}px) / 2)`;
export const DESKTOP_CHIP_W = 168;
export const CHIP_W_CSS_DESKTOP = `${DESKTOP_CHIP_W}px`;
export const RIBBON_FADE = "linear-gradient(90deg, #000 0, #000 calc(100% - 12px), transparent 100%)";

export const MARKER_AVATAR_R = 13;
export const MARKER_ANCHOR_R = 3.6;
export const AVATAR_BG = "#3D3D3D";
